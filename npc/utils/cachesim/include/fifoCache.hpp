...
template <typename addr_t>
FIFOCache<addr_t>::FIFOCache(unsigned int _e, unsigned int _s, unsigned int _b) : Cache<addr_t>(_e, _s, _b) {
    counter = std::vector<uint32_t>(this->S, 0);
}

template <typename addr_t>
unsigned int FIFOCache<addr_t>::get_replace_entry(unsigned int groupIndex) {
    uint32_t t = counter[groupIndex];
    counter[groupIndex] = (counter[groupIndex] + 1) % this->E;
    return t;
}
